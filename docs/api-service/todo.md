...
- [x] `GET /objects/{objectId}` — агрегаты (расходы) в ответе.
...
- [x] Реализовать правило видимости расходов: только автор + участники подтверждения.
- [x] Зафиксировать статусы расхода и переходы (draft → pending → approved/rejected, cancel).
- [x] Сценарий подтверждения: участники, история (пока параллельно).

### Эндпоинты

- [x] `GET /expenses` — фильтры по объекту/категории/типу/статусу/периоду.
- [x] `POST /expenses` — создание расхода (вложения чеков, сценарий подтверждения).
- [x] `GET /expenses/{expenseId}` — карточка + вложения + состояние подтверждений.
- [x] `PATCH /expenses/{expenseId}` — редактирование автором до завершения подтверждений.
- [x] `POST /expenses/{expenseId}/submit` — из черновика в ожидание подтверждения.
- [x] `POST /expenses/{expenseId}/approve` — подтвердить.
- [x] `POST /expenses/{expenseId}/reject` — отклонить с комментарием.
- [x] `POST /expenses/{expenseId}/cancel` — отменить автором.
